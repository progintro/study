...
### Αναπαράσταση πεδίων bit στη μνήμη
...
### Εύρος τιμών και ανάθεση σε πεδία bit
...
### Μέγεθος και κοινή μνήμη της ένωσης
...
### Το μέγεθος μιας δομής με πεδία bit
...
### Ακολουθώντας τον γονικό φάκελο
...
- **Σημειώσεις:** η διάλεξη προτείνει τις σελ. 108 και 120–135 των διαφανειών του κ.
  ...
  - [Κεφάλαιο 7: Απαριθμήσεις, δομές και ενώσεις](https://progintro.github.io/notes/chapters/07-structs/):
    «Απαριθμήσεις» (K04, σελ. 109), «Αυτο-αναφορικές δομές» (121–126), «Δημιουργία
    νέων ονομάτων τύπων» (127), «Ενώσεις και πεδία bit» (128).
  - [Κεφάλαιο 8: Λίστες και δυαδικά δέντρα](https://progintro.github.io/notes/chapters/08-lists-trees/):
    «Διαχείριση συνδεδεμένων λιστών» (K04, σελ. 129–132), «Διαχείριση δυαδικών
    δέντρων» (133–136), για την επόμενη διάλεξη.
...
3. Γιατί δεν μπορείτε να γράψετε `scanf("%u", &st.year);` όταν το `year` είναι πεδίο
   bit;[^q3]
4. Πόσο είναι το `sizeof` μιας `union` με μέλη `char`, `int` και `double`, και
   γιατί;[^q4]
5. Στο `enum color {RED, GREEN = 5, BLUE};` τι τιμή έχει το `BLUE`;[^q5]
6. Γιατί η `struct folder` περιέχει `struct folder *parent` και όχι
   `struct folder parent`;[^q6]
7. Πώς καταλαβαίνει ένας βρόχος που ακολουθεί τους δείκτες `parent` ότι έφτασε στη
   ρίζα;[^q7]
